...
class Customer:
    ...
    def pays_bill(self,bill):
        ...
        #print("customer name",self.__customer_name,"bill id",bill.get_bill_id(),"bill amount",bill.get_bill_amount())
...
class Bill:
    ...
    def generate_bill_amount(self,item_quantity,items):
        ...
        for key,values in item_quantity.items():
            for item in items:
                self.__item_id = item.get_item_id()
                if self.__item_id.lower() == key.lower():
                    self.__bill_amount += values * item.get_price_per_quantity()
