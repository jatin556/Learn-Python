...
class Ticket:
    ...
    def calculate_amount(self, list_of_games):
        ...
        for game in list_of_games:
            for key in ThemePark.dict_of_games.keys():
                if key == game:
                    amount += ThemePark.get_amount(game)
                    self.__ticket_amount = amount
                    templ.append(game)
        if len(templ)== len(list_of_games):
            return True
        return False
        #Remove pass and write the logic here
...
class Customer:
    ...
    def play_game(self):
        ...
        for game in self.__list_of_games:
            for key in ThemePark.dict_of_games.keys():
                if key == game:
                    points += ThemePark.get_points(game)
                    
            if game == "Game3":
                if "Game2" not in self.__list_of_games:
                    self.__list_of_games.append("Game2")
#                     points += ThemePark.dict_of_games["Game2"][1]
